...
import com.fss.aeps.services.AcquirerReversalService;
...
public class RevPaySender implements IAcquirerTransaction<ReqPay, RespPay>{

	@Autowired
	private AppConfig appConfig;

	@Autowired
	private AcquirerReversalService reversalService;
	...
	public RespPay send(ReqPay request) {
		...
		final AcquirerReversal reversal = (AcquirerReversal) request.context.get(ContextKey.ACQUIRER_REVERSAL);
		reversalService.registerReversalRequest(reversal, request);
		...
		try {
			...
			logger.info("acquirer request ack received.");
			...
			try {
				...
				reversalService.registerReversalResponse(reversal, request, response);
